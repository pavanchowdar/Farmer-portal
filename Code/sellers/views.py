...
from .models import SellerUserRegistrationModel, FarmersCropsModels,FarmersCropDataModels
...
def SellerReportForm(request):
    if request.method == 'POST':
        cropname=request.POST.get('crop')
        reports = FarmersCropsModels.objects.filter(cropname=cropname.lower())
        return render(request,'sellers/report.html',{'reports':reports})
    else:
        cropDetails = FarmersCropsModels.objects.all()
        return render(request,'sellers/report.html',{'cropDetails':cropDetails})
...
def SellerAddItemsAction(request):
    if request.method=='POST':
        ...
        FarmersCropsModels.objects.create(sellername=loginid, selleremail=email, cropname=cropname.lower(),price=price, description=description,file=uploaded_file_url)
